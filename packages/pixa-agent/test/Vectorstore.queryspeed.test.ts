...
// ---------------------------------------------------------------------------
// Replaces the old test, which pointed VectorStore at a real workspace
// (TelemetryX) whose on-disk .pixa/index was truncated/corrupted. That made
// the test depend on machine-specific state instead of testing our code, and
// it wasn't actually measuring query speed — it was crashing during init()
// before a query ever ran.
//
// This version is fully self-contained:
//   - uses a throwaway temp directory as the "workspace" (no real repo state)
//   - mocks ../providers/embeddings so no real transformer model is ever
//     loaded — this test is about VectorStore.query() latency, not model
//     cold-start time, and a real model load would make it slow *and* flaky
//     in CI
//   - the mock embedding is a small deterministic bag-of-words hash, so
//     relevance ranking is still meaningfully testable (semantically similar
//     text scores higher), not just "does it not throw"
// ---------------------------------------------------------------------------
...
// Imported after the mock so VectorStore picks up the mocked embed().
const { VectorStore } = await import("../src/indexer/vectorStore");
const { embed } = await import("../src/providers/embeddings");
